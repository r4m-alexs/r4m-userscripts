/* r4m.bundle.js — GENERATED by build-insomnia-r4m.js from postman.package.js; do not edit.
   Load in an Insomnia Pre-request Script via insomnia-r4m/loader.js (fetch a hosted URL + eval),
   or self-contained:  const r4m = eval(insomnia.environment.get('r4m'));
   HTTP = native fetch; config = request headers/env; deps (lodash/moment/csv-parse) via require(). */
(function () {
'use strict';
// native-bindings.js — the ONLY non-1:1 pieces of the Insomnia port. Everything else in the library
// is the literal native `insomnia.*` API (environment/globals/collectionVariables/request/visualizer).
// These three exist because Insomnia has no native equivalent:
//   __send     — Postman's pm.sendRequest has no native twin; this is native `fetch` shaped to match.
//   __cfg/__params — config/secrets read from request HEADERS first, then the Insomnia environment.
//   __vaultGet — Insomnia has no Vault; secrets come from the environment (header-or-env).

// header-first config: first non-empty header among the names, else the environment (first name)
function __hdr(n) { try { return insomnia.request.headers.get(n); } catch (e) { return undefined; } }
function __envGet(k) { try { return insomnia.environment.get(k); } catch (e) { return undefined; } }
function __cfg() { for (var i = 0; i < arguments.length; i++) { var h = __hdr(arguments[i]); if (h != null && h !== '') return h; } var v = __envGet(arguments[0]); return v == null ? undefined : v; }
function __vaultGet(k) { return Promise.resolve(__cfg(k)); }

// the request params object the library reads as `query` — native query params, overlaid with
// header config (env / api_key (X-API-KEY) / debug / include / route_id / member_id) when present.
function __params() {
    var q;
    try { q = insomnia.request.url.query.toObject(true); } catch (e) { q = {}; }
    var over = { env: __hdr('env'), api_key: __cfg('api_key', 'X-API-KEY'), debug: __hdr('debug'), include: __hdr('include'), route_id: __hdr('route_id'), member_id: __hdr('member_id') };
...
// downstream sync .text()/.json() calls work unchanged.
function __send(options, cb) {
    try {
        var o = (typeof options === 'string') ? { url: options } : (options || {});
        var method = (o.method || 'GET').toUpperCase();
        var headers = {};
        if (o.header) for (var k in o.header) { if (o.header[k] != null) headers[k] = String(o.header[k]); }
        var body;
        if (o.body) {
        ...
        fetch(o.url, { method: method, headers: headers, body: (method === 'GET' || method === 'HEAD') ? undefined : body })
            .then(function (res) {
...
let     query = __params(),
        path = insomnia.request.url.path,
        response = insomnia?.response?.headers.get('Content-Type')?.match('application/json') ? insomnia?.response?.json() : insomnia?.response?.text(),
        PROD = query.env === 'prod',
        ...
        creds = insomnia.globals.get('setBillingCreds') ? JSON.parse(insomnia.globals.get('setBillingCreds')) : {}; 
...
let request = async ({ 
    ...
    return new Promise((resolve, reject) => {
        ...
        const sendOnce = () => new Promise((res) => __send(options, (err, resp) => res({ err, resp })));
...
const from = (data) => {
    ...
    const api = {
        ...
        visualize(title = null) {
            let r = run();
            try {
                if (typeof insomnia !== 'undefined' && insomnia.visualizer && typeof insomnia.visualizer.set === 'function') {
                    ...
                    insomnia.visualizer?.set?.(tpl, { rows: r, cols, title });
...
const _cvStr = (name) => { let c = insomnia.collectionVariables.get(name); return (typeof c === 'string' && c && c !== '[object Object]') ? c : null; };
// config value: request HEADERS first (any of the given names), else the URL query (first name)
const cfg = (...names) => {
    for (let n of names) { try { let h = insomnia.request.headers.get(n); if (h != null && h !== '') return h; } catch (e) {} }
...
const resolveApiKey = async (api_key, env) => {
    ...
    if (api_key) {                                     // explicit arg wins; ensure {{token}} matches
        if (_cvStr('token') !== api_key) insomnia.collectionVariables.set('token', api_key);
        return api_key;
    }
    ...
    if (isPROD) {
        ...
        if (key) { if (cached !== key) insomnia.collectionVariables.set('token', key); return key; }
    ...
    if (member_id) {
        ...
        if (bearer && bearer !== cached) insomnia.collectionVariables.set('token', bearer);
        return bearer || null;
    }
    if (key) { if (cached !== key) insomnia.collectionVariables.set('token', key); return key; }
...
const validateSchema = (entity, code, items, { force = false, show = false } = {}) => {
    ...
    let saved = null; try { let raw = insomnia.collectionVariables.get(key); if (raw) saved = JSON.parse(raw); } catch (e) {}

    if (force || !saved) {
        insomnia.collectionVariables.set(key, JSON.stringify(current));
...
const paginate = ({ data = null, per_page = null, scope = 'collection', cursorVar = 'cursor', pageVar = 'pagenum', reset = false } = {}) => {
    let store = scope === 'globals' ? insomnia.globals : (scope === 'environment' ? insomnia.environment : (insomnia.collectionVariables || insomnia.variables));
    if (reset) {
        store.set(cursorVar, '');
        store.set(pageVar, '1');
...
const vault = async (key) => { try { return await __vaultGet(key); } catch (e) { console.error('vault: missing/unreadable "' + key + '"'); return null; } };
...
const clearFeaturesCache = (env = null) => {
    ...
    names.forEach(n => { try { insomnia.globals.unset ? insomnia.globals.unset(n) : insomnia.globals.set(n, ''); } catch (e) {} });
};
...
const features = async ({ env = null, renew = false, api_key = null } = {}) => {
    ...
    let cached = insomnia.globals.get(cacheName);
    ...
    if (!renew && fresh) {
        ...
        ts = now();
        insomnia.globals.set(cacheName, JSON.stringify({ timestamp: ts, list }));
    }
...
const _domainState = (url) => new Promise((resolve) => {
    ...
    try {
        ...
        let jar = insomnia.cookies && insomnia.cookies.jar && insomnia.cookies.jar();
...
const now = () => {
    ...
    insomnia.globals.set("now", timestamp);
    return timestamp;
}
...
const clearCookies = (domain) => { insomnia.cookies?.jar().clear(domain); };
...
const clearAuth = () => { insomnia.collectionVariables.unset('api_key'); insomnia.collectionVariables.unset('token'); };
...
const endpoint = async (params) => {
    let baseUrl,
        api_key,
        member_id;
    insomnia.request.headers.upsert({
        key: 'X_R4M_TEST_ID',
    ...
    if (baseUrl) insomnia.collectionVariables.set("baseUrl", baseUrl);
    ...
    if (api_key) {
        if (cached !== api_key) insomnia.collectionVariables.set('token', api_key);
        return;
    }
    if (member_id) {
        insomnia.collectionVariables.set('member_id', member_id);
        ...
        if (!cached) {   // staging: mint only when {{token}} is empty
            ...
            if (bearer) insomnia.collectionVariables.set('token', bearer);
        }
        return;
    }
...
const variable = (variable, value, plain = false) => {
    if (typeof value === 'object') {
        return insomnia.collectionVariables.set(variable, JSON.stringify(value));
    } else {
        return insomnia.collectionVariables.set(variable, value);
    }
}
...
const visualize = (data) => {
    ...
    <link href=" https://cdn.jsdelivr.net/npm/json-tree-viewer@0.0.2/libs/jsonTree/jsonTree.min.css " rel="stylesheet"><head>
                <body>
                    <script type="text/javascript">
                        insomnia.getData( function(err, payload) {
    ...
    insomnia.visualizer?.set?.(template, { data: data });
};
...
const jiraCreds = async () => {
    let jiraAPIKEY = await __vaultGet('jira');
